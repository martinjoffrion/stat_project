...
Notre modèle est déjà filtré sur une espèce, et l'ANOVA intègre les variables relevé et altitude, la seule variable encore suscueptible de générer une dépendance des résidus est le statut, à savoir l'état de l'arbre au moment du prélèvement.
...
Notre modèle n'est plus filtré sur une espèce, mais l'ANOVA intègre toutes les variables potentiellement sources de dépendances des erreurs. Ainsi, les résidus ont un biais nul quant à la variable espèce, comme prouvé sur le boxplot.
...
Comparé aux résultats obtenu avec la variable latitude, la variable *lasLog* (dernière coupe massive) explique bien mieux l'effet relevé. C'est notamment visible dans le Scale-Location plot, où la moyenne glissée suit presque parfaitement le moyenne théorique, à l'exception des individus aux forts diamètres. 
...
Pour affirmer ces résultats, nous avons calculé le R² de McFadden, afin d'évaluer le progrès apporté en terme de vraisemblance par notre modèle avec covariables. 
...
Dans le cours, le R² de McFadden obtenu avec un modèle avec altitude seulement était de 0.188, soit très légèrement inférieur au notre obtenu avec l'ajout du DBH, confirmant encore une fois le faible impact du DBH sur le modèle.